...
int main(int argc, char *argv[]){
    ...
    // fclose(file);
    // int res = solve(schools, nb_schools, students, nb_students);
    // if (res == -1){
    //     fprintf(stderr, "Error: couldnt solve the problem\n");
    //     exit(EXIT_FAILURE);
    // }
    // for(int i = 0; i < nb_students; i++){
    //     printf("%s %s\n", getName(students[i]), getName(getChosen(students[i])));
    // }
    // for(int i = 0; i < nb_students; i++){
    //     freeEntity(students[i]);
    // }
    // for(int i = 0; i < nb_schools; i++){
    //     freeEntity(schools[i]);
    // }
    // free(students);
    // free(schools);
    // return 0;